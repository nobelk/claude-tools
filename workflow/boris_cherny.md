...
[Reference](https://x.com/bcherny/status/2007179832300581177?ref_src=twsrc%5Etfw%7Ctwcamp%5Etweetembed%7Ctwterm%5E2007179832300581177%7Ctwgr%5Ea687b9882023a163db469c679f4f1a6025562a0a%7Ctwcon%5Es1_&ref_url=https%3A%2F%2Fventurebeat.com%2Ftechnology%2Fthe-creator-of-claude-code-just-revealed-his-workflow-and-developers-are)
...
1. Run 5 Claudes in parallel in my terminal. Number the tabs 1-5, and use system notifications to know when a Claude needs input [terminal setup](https://code.claude.com/docs/en/terminal-config#iterm-2-system-notifications
)

2. We can also run 5-10 Claudes on [claude.ai](http://claude.ai/code), in parallel with my local Claudes. As I code in my terminal, I will often hand off local sessions to web (using &), or manually kick off sessions in Chrome, and sometimes I will --teleport back and forth. I also start a few sessions from my phone (from the Claude iOS app) every morning and throughout the day, and check in on them later.

3. Use Opus 4.5 with thinking for everything. It's the best coding model I've ever used, and even though it's bigger & slower than Sonnet, since you have to steer it less and it's better at tool use, it is almost always faster than using a smaller model in the end.
...
Other teams maintain their own [claude.md](http://CLAUDE.md). It is each team's job to keep theirs up to date.
![claude-md](claude-md.jpeg)
...
![post-tool-hook](post-tool-hook.jpeg)
